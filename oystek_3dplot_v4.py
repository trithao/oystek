...
#   adjust, maxDiagonal, len_nlines_ratio, dots_count, volume, projection, edges
#===========================================
#===========================================
...
def oystek_reset(fname):
  global oyster_images, oyster_features, oyster_velocity, oyster_length, oyster_grade, \
         fin, fout, current_line, num_oysters, scatter_x, scatter_y, contour_x, contour_y, \
         adjust, maxDiagonal, len_nlines_ratio, dots_count, volume, rax, check, \
         projection, edges
...
# rotate (x,y) by an angle a (in radian) around the origin (0,0) on XY plan
def rotate(a, x, y):
...
def point_on_contour(xy,z,i):
  ...
  y = xy[1]
  if i==0:
    return (oyster_images[0][z][ x+n_cols/2] == '#') and (is_on_contour(0,z, x+n_cols/2) or \
            is_on_contour(0, z,            x+n_cols/2-wire_thick) or is_on_contour(0,  z,            x+n_cols/2+wire_thick) or \
            is_on_contour(0, z+wire_thick, x+n_cols/2)            or is_on_contour(0,  z-wire_thick, x+n_cols/2) or \
            is_on_contour(0, z+wire_thick, x+n_cols/2+wire_thick) or is_on_contour(0,  z+wire_thick, x+n_cols/2-wire_thick) or \
            is_on_contour(0, z-wire_thick, x+n_cols/2+wire_thick) or is_on_contour(0,  z-wire_thick, x+n_cols/2-wire_thick) )
  elif i==2:
    return (oyster_images[2][z][-y+n_cols/2] == '#') and (is_on_contour(2,z,-y+n_cols/2) or \
            is_on_contour(2, z,           -y+n_cols/2-wire_thick) or is_on_contour(2,  z,           -y+n_cols/2+wire_thick) or \
            is_on_contour(2, z+wire_thick,-y+n_cols/2)            or is_on_contour(2,  z-wire_thick,-y+n_cols/2) or\
            is_on_contour(2, z+wire_thick,-y+n_cols/2+wire_thick) or is_on_contour(2,  z+wire_thick,-y+n_cols/2-wire_thick) or \
            is_on_contour(2, z-wire_thick,-y+n_cols/2+wire_thick) or is_on_contour(2,  z-wire_thick,-y+n_cols/2-wire_thick))
  elif i==1:
    return (oyster_images[1][z][-x+n_cols/2] == '#') and (is_on_contour(1,z,-x+n_cols/2) or \
            is_on_contour(1, z,           -x+n_cols/2-wire_thick) or is_on_contour(1,  z,           -x+n_cols/2+wire_thick) or \
            is_on_contour(1, z+wire_thick,-x+n_cols/2)            or is_on_contour(1,  z-wire_thick,-x+n_cols/2) or \
            is_on_contour(1, z+wire_thick,-x+n_cols/2+wire_thick) or is_on_contour(1,  z+wire_thick,-x+n_cols/2-wire_thick) or \
            is_on_contour(1, z-wire_thick,-x+n_cols/2+wire_thick) or is_on_contour(1,  z-wire_thick,-x+n_cols/2-wire_thick) )
  elif i==3:
    return (oyster_images[3][z][-y+n_cols/2] == '#') and (is_on_contour(3,z,-y+n_cols/2) or \
            is_on_contour(3, z,           -y+n_cols/2-wire_thick) or is_on_contour(3,  z,           -y+n_cols/2+wire_thick) or \
            is_on_contour(3, z+wire_thick,-y+n_cols/2)            or is_on_contour(3,  z-wire_thick,-y+n_cols/2) or \
            is_on_contour(3, z+wire_thick,-y+n_cols/2+wire_thick) or is_on_contour(3,  z+wire_thick,-y+n_cols/2-wire_thick) or \
            is_on_contour(3, z-wire_thick,-y+n_cols/2+wire_thick) or is_on_contour(3,  z-wire_thick,-y+n_cols/2-wire_thick) )
  return False
...
def edge_toggle(label):
    if label == 'red':
      edges[0].set_visible(not edges[0].get_visible())
    elif label == 'green':
      edges[1].set_visible(not edges[1].get_visible())
    elif label == 'blue':
      edges[2].set_visible(not edges[2].get_visible())
    elif label == 'black':
      edges[3].set_visible(not edges[3].get_visible())
    elif label == 'cyan':
      edges[4].set_visible(not edges[4].get_visible())
    elif label == 'orange':
      edges[5].set_visible(not edges[5].get_visible())
    elif label == 'grey':
      projection[0].set_visible(not projection[0].get_visible())
      projection[1].set_visible(not projection[1].get_visible())
      projection[2].set_visible(not projection[2].get_visible())
      projection[3].set_visible(not projection[3].get_visible())
      projection[4].set_visible(not projection[4].get_visible())
      projection[5].set_visible(not projection[5].get_visible())
      projection[6].set_visible(not projection[6].get_visible())
      projection[7].set_visible(not projection[7].get_visible())
    plt.draw()



def plot_oyster_3Dimages():
    global volume, check, rax, edges, projection
    volume = [[[],[],[]], [[],[],[]], [[],[],[]], [[],[],[]], [[],[],[]], [[],[],[]]]
    #volume = [[[],[],[]]] * 6
    ...
    # images 1 & 3
    projection[0] = ax.scatter(contour_x[0]-n_cols*np.ones(len(contour_x[0]))/2, -radius*np.ones(len(contour_x[0])), contour_y[0], s=1, color='grey')
    ax.text(0, -radius, 0, "1", color='black')
    projection[1] = ax.scatter(contour_x[0]-n_cols*np.ones(len(contour_x[0]))/2,  radius*np.ones(len(contour_x[0])), contour_y[0], s=1, color='lightgrey')
    ax.text(0, radius, 0, "1", color='lightgrey')
    
    projection[2] = ax.scatter( radius*np.ones(len(contour_x[2])), n_cols*np.ones(len(contour_x[2]))/2 - contour_x[2], contour_y[2], s=1, color='grey')
    ax.text(radius, 0, 0, "3", color='black')
    projection[3] = ax.scatter(-radius*np.ones(len(contour_x[2])), n_cols*np.ones(len(contour_x[2]))/2 - contour_x[2], contour_y[2], s=1, color='lightgrey')
    ax.text(-radius, 0, 0, "3", color='lightgrey')
    ...
    projection[4] = ax.scatter(xx2, yy2, contour_y[1], s=1, color='grey')
    ...
    projection[5] = ax.scatter(xx2p, yy2p, contour_y[1], s=1, color='lightgrey')
    ax.text(-radius*np.cos(np.pi/4), radius*np.cos(np.pi/4), 0, "2", color='lightgrey')

    xx4,  yy4  = rotate(np.pi/4,  radius*np.ones(len(contour_x[3])), n_cols*np.ones(len(contour_x[3]))/2-contour_x[3])
    projection[6] = ax.scatter(xx4,  yy4,  contour_y[3], s=1, color='grey')
    ...
    projection[7] = ax.scatter(xx4p, yy4p, contour_y[3], s=1, color='lightgrey')
    ax.text(-radius*np.cos(np.pi/4), -radius*np.cos(np.pi/4), 0, "4", color='lightgrey')
    ...
    for x,y,z in zip(xxx,yyy,zzz):
      ...
      # intersection 1 & 3 : CYAN
      if (point_on_contour([x,y],-z,0) and point_on_contour([x,y],-z,2) and point_in_view([int(x1i),int(y1i)],-z,1) and point_in_view([int(x1i),int(y1i)],-z,3)):
        volume[4][0].append(x)
        volume[4][1].append(y)
        volume[4][2].append(z)

      # intersection 2 & 4 : ORANGE
      if (point_on_contour([int(x1i),int(y1i)],-z,1) and point_on_contour([int(x1i),int(y1i)],-z,3) and point_in_view([x,y],-z,0) and point_in_view([x,y],-z,2)):
        volume[5][0].append(x)
        volume[5][1].append(y)
        volume[5][2].append(z)
        
        
    rax = plt.axes([0.05, 0.1, 0.1, 0.25])
    check = CheckButtons(rax, ('red', 'green', 'blue', 'black', 'cyan', 'orange', 'grey'), (True, True, True, True, True, True, True))
    check.labels[0].set(color='red')
    check.labels[1].set(color='green')
    check.labels[2].set(color='blue')
    check.labels[3].set(color='black')
    check.labels[4].set(color='cyan')
    check.labels[5].set(color='orange')
    check.labels[6].set(color='grey')
    check.on_clicked(edge_toggle)

    
    edges[0] = ax.scatter(volume[0][0], volume[0][1], volume[0][2], s=point_size, color='red' )
    edges[1] = ax.scatter(volume[1][0], volume[1][1], volume[1][2], s=point_size, color='green' )
    edges[2] = ax.scatter(volume[2][0], volume[2][1], volume[2][2], s=point_size, color='blue' )
    edges[3] = ax.scatter(volume[3][0], volume[3][1], volume[3][2], s=point_size, color='black' )
    edges[4] = ax.scatter(volume[4][0], volume[4][1], volume[4][2], s=point_size, color='cyan' )
    edges[5] = ax.scatter(volume[5][0], volume[5][1], volume[5][2], s=point_size, color='orange' )
...
oystek_start("28Mar13\\73mm Oyster Random 2013-Mar-28.log")
